...
class RenamerTest {
    private final BsonDocument keyDoc = BsonDocument.parse("{'fieldA': 'my field value', 'f2': true, 'subDoc': {'fieldX': 42}, "
            + "'my_field1': {'my_field2': 'testing rocks!'}}");
    private final BsonDocument valueDoc = BsonDocument.parse("{'abc': 'my field value', 'f2': false, 'subDoc': {'123': 0.0},"
    ...
    void testRenamerUsingFieldnameMapping() {
        BsonDocument expectedKeyDoc = BsonDocument.parse("{'f1': 'my field value', "
                + "'fieldB': true, 'subDoc': {'name_x': 42}, 'my_field1': {'my_field2': 'testing rocks!'}}");
        BsonDocument expectedValueDoc = BsonDocument.parse("{'xyz': 'my field value', 'f_two': false, "
                + "'subDoc': {'789': 0.0}, 'foo.foo.foo': {'.blah..blah.': 23}}");
        MongoSinkTopicConfig cfg = createTopicConfig(FIELD_RENAMER_MAPPING_CONFIG,
    ...
    void testRenamerUsingRegExpSettings() {
        BsonDocument expectedKeyDoc = BsonDocument.parse("{'FA': 'my field value', 'f2': true, "
                + "'subDoc': {'FX': 42}, '_F1': {'_F2': 'testing rocks!'}}");

        BsonDocument expectedValueDoc = BsonDocument.parse("{'abc': 'my field value', 'f2': false, "
                + "'subDoc': {'123': 0.0}, 'foo_foo_foo': {'_blah__blah_': 23}}");

        MongoSinkTopicConfig cfg = createTopicConfig(FIELD_RENAMER_REGEXP_CONFIG,
